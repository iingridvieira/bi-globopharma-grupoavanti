-- Pendências (pedidos em aberto) do BI IMEC — mesma ideia da "pendencias_produtos"
-- do BI Globo Pharma, em tabela própria (prefixo "imec_") para não encostar em
-- nada do Globo. Snapshot único (sem mês/ano): cada importação substitui a base
-- da(s) empresa(s) presentes no arquivo, como a pendência do Globo.
