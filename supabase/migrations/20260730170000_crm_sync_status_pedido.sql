-- Ajusta a integração CRM <-> BI para respeitar o status do Pedido Enviado
-- (public.pedidos_enviados.status: 'aguardando' ou 'aprovado', já existente no BI):
--   'aguardando' -> CRM: status do mês = "negociacao" (ainda não é uma compra confirmada)
--   'aprovado'   -> CRM: status do mês = "comprou" + registra em crm_compras (última compra)
-- O trigger passa a disparar também quando o status do pedido muda (aprova/reabre).
...
BEGIN
  ...
  -- Só registra como compra (e atualiza "Última compra") quando o pedido está aprovado.
